...
void Screen::drawAll() {
    // std::cout << "Before Draw All" << std::endl;

    glClearColor(mBackground[0], mBackground[1], mBackground[2], mBackground[3]);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);

    // std::cout << "Before Draw Contents" << std::endl;
    drawContents();
    // std::cout << "After Draw Contents" << std::endl;
    drawWidgets();
    // std::cout << "After Draw Widgets" << std::endl;

    // std::cout << "Before Post Draw" << std::endl;
    postDraw();
    // std::cout << "After Post Draw" << std::endl;

    glfwSwapBuffers(mGLFWWindow);

    // std::cout << "After Swap Buffs" << std::endl;

}

void Screen::postDraw() {
    // does nothing only extended
}

void Screen::drawWidgets() {
    // std::cout << "WHAT" << std::endl;
    if (!mVisible) {
        // std::cout << "NOT VIS" << std::endl;
        return;
    }
...
bool Screen::keyboardEvent(int key, int scancode, int action, int modifiers) {
    if (mFocusPath.size() > 0) {
        for (auto it = mFocusPath.rbegin() + 1; it != mFocusPath.rend(); ++it) {
            // std::cout << "WHATS: " << mFocusPath.size() << std::endl;
